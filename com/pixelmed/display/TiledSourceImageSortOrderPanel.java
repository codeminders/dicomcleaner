...
class TiledSourceImageSortOrderPanel extends SourceInstanceSortOrderPanel {
	...
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/display/TiledSourceImageSortOrderPanel.java,v 1.13 2018/06/03 05:17:26 dclunie Exp $";
	...
	protected void updateColumnsSlider(int min,int max,int value) {
		...
		if (min != currentColumnsSliderMinimum || max != currentColumnsSliderMaximum) {
			...
			if (max > min) {
				...
				columnsSlider=null;	// else single frame so no slider
			}
	...
	protected void updateRowsSlider(int min,int max,int value) {
		...
		if (min != currentRowsSliderMinimum || max != currentRowsSliderMaximum) {
			...
			if (max > min) {
				...
				rowsSlider=null;	// else single frame so no slider
			}
			currentSliderMinimum=min;
			currentSliderMaximum=max;
		}
